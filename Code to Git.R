## Cleaned Analysis Pipeline for Note Events and Diagnosis Outcomes
## Author: [Your Name] | Date: 2025-07-15
...
raw_notes <- read.csv2("NOTEEVENTS (1).csv", sep = ",", stringsAsFactors = FALSE)
...
# Build master dataset: one row per patient-admission including flags
...
final_disease_data %>%
  ...
  count(diab, word, sort = TRUE) %>%
  filter(nchar(word)>2) %>%
  bind_log_odds(diab, word, n) %>% 
  View()
...
final_disease_data %>%
  unnest_tokens(word, TEXT) %>%
  count(surren, word, sort = TRUE) %>%
  anti_join(get_stopwords()) %>%
  filter(nchar(word)>2) %>%
  bind_log_odds(surren, word, n) 
...
# ordered_diab <- ordered_diab[1:50]
# DTM_byhand <- DTM_byhand[1:50,]
...
#pdf('rate_sugar.pdf', width = 6, height = 8)
...
#Y_hyp_test <- sample(Y_hyp_test,15)
...
# ordered_surr <- ordered_surr[1:50]
...
#Y_surr_test <- sample(Y_surr_train,15)
